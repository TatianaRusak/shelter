...
let suffeledArr = [];
for (let i = 0; i < Math.ceil(arrOf48Numbers.length / numberOfcardsPerPage); i++) {
    subarray[i] = arrOf48Numbers.slice((i * numberOfcardsPerPage), (i * numberOfcardsPerPage) + numberOfcardsPerPage);
    shuffle(subarray[i]);
...
function shuffle(array) {    // функция для перемешивания чисел в массиве
...
sliderButtons.addEventListener('click', () => {
    ...
    if(currentPageNumber.innerHTML == Math.ceil(arrOf48Numbers.length / numberOfcardsPerPage)){
